// Canonical, pure, unit-tested model-routing dispatch for cw-orchestrate.
//
// The Fable-pinned plan step judges each sub-issue against the shared
// complexity rubric (references/complexity-rubric.md) and emits a routing
// block { provider, model, effort, complexity, rationale }. This helper turns
// that block into the `agent()` opts for the downstream work/autofix agent.
//
 ...
 * or no routing at all (e.g. an autofix whose parent node's plan is missing)
 * — defaults UP to opus per the rubric's route-up bias. An invalid effort is
 * dropped (the harness default applies) rather than guessed.
 *
