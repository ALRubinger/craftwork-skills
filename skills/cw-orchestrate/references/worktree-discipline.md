...
- **The post-run cleanup heals the primary checkout** — `cw-ship` Step 5, `cw-sweep` Step 5, `cw-orchestrate` Step 8 remove the run's worktrees (merge-state gated) and fast-forward the primary checkout's default branch back to `origin`.
