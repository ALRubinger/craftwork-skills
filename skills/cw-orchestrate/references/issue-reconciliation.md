...
2. **Reflect sub-issue state natively — no checklists.** A cw-skill umbrella tracks its children as GitHub **native sub-issues**, never a body checklist, so there is no checkbox to tick: a resolved child is simply **closed** (the native sub-issue widget renders it struck-through and rolls the count up automatically). A still-open child that the run left parked carries a status label so the umbrella's sub-issue list reads true without any body edit:
   ...
   **Milestones are the canonical case of this exception, and they reconcile by hand.** A milestone is the third tier above the umbrella (`milestone → umbrella → sub-issue`; see cw-scope's issue-templates) — a deliberately human-curated `- [ ] #NNN` checklist, *not* native sub-issues, because its thematic sections, per-line annotations, dated scope notes, and readiness flags are load-bearing and have no native equivalent. When a child umbrella/issue this run resolved is listed on a milestone:
   - **Tick + annotate its line in the right thematic section.** Flip `- [ ]` to `- [x]` on that child's line, keep it under the same theme heading, and append the resolution annotation in the milestone's grammar — `— merged via #<pr>` (or `— re-parented to #NNN` if the child moved). Don't relocate the line or restructure sections; the curation is the milestone's value.
   - **NEVER auto-close a milestone.** Even when ticking this line makes every box checked, closing a milestone is a human roadmap call (recorded walkthroughs, demo scripts, and acceptance the milestone gates often live outside the checklist) — unlike an umbrella, which *is* auto-closed when its children are all done (rule 3). Leave the milestone open; at most post a one-line progress comment.
   - **NEVER convert a milestone to native sub-issues.** Do not run the porting recipe or `addSubIssue` against a milestone's children. The checklist is intentional, and converting it would destroy the annotations and grouping — this protects the milestone from a future native-migration sweep that would otherwise treat its `- [ ] #NNN` lines as a legacy umbrella to port.

3. **Consult parents — close them when (and only when) everything under them is done.** If resolving this issue means a parent's children are *all* resolved and nothing is deferred / stalled / a live escalation, close the parent too (`gh issue close <parent> --reason completed --comment "<one-line outcome>"`). Otherwise leave it open and, when useful, post a progress comment so the parent reflects partial completion. Never close a parent that still tracks open work. **Exception: a milestone parent is never auto-closed** — closing it is a human roadmap call (see rule 2's milestone clause), so leave it open even when every box is checked.
