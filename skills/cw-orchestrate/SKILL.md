...
- **`<repo>` — an `owner/repo` slug** (contains a `/` and is not a full issue URL, e.g. `ALRubinger/craftwork-skills`) → **repo-scan mode**. Enumerates every OPEN issue in that repo carrying the `cw-umbrella:ready` label and runs each one, serially, through the per-umbrella flow — but with **no interactive sweep** (the label is the upstream clearance — human approval for a cw-scope umbrella, cw-ship's triage judgment for one it filed; see [Repo-scan mode](#repo-scan-mode-headless-label-driven-pickup) below). This is the on-demand headless entry point: you invoke `/cw-orchestrate <owner>/<repo>` yourself (e.g. via `claude -p`) to scan the repo for ready umbrellas and drive each one to merge.
...
`/cw-orchestrate <owner>/<repo>` runs **without** the interactive sweep. It trusts the upstream clearance the `cw-umbrella:ready` label already encodes (stamped at interactive scoping for cw-scope umbrellas, or by cw-ship's triage judgment that the feedback was umbrella-sized with clear intent — no `cw-feedback:go` gate, since a genuine design fork would have parked instead of producing an umbrella) and cannot solicit a human headless. This is the on-demand headless repo-scan entry path; the gate posture is documented in [readiness-sweep.md](./references/readiness-sweep.md#two-gate-postures).
...
2. **Per umbrella, apply the live-state in-flight guard.** Because the label persists through an in-flight run (it is only removed when the umbrella is fully resolved), a naïve scan could double-run an umbrella another orchestrate run is already working. Before running a scan-picked umbrella, check its **live state** for an in-flight orchestration: an open PR that closes one of its sub-issues, or a sub-issue mid-work (e.g. an open linked PR). If one is found, **skip this umbrella this scan** and name it in the report as "in flight, skipped." Two orchestrate runs on one umbrella at once stays explicitly unsupported (as in number mode); this guard is how the repo-scan path avoids it without introducing a lock or mirror label.

   **Best-effort, not a lock — the launch→first-PR window.** The guard keys off signals a *running* orchestration emits (an open PR, a sub-issue mid-work). Between launching a run and its first PR opening, no such signal exists yet, so the guard is **blind to a run in that window**. That launch→first-PR window is safe **only** under a single-process sequential run — one invocation at a time, each umbrella carried to completion before the next. Two concurrent repo-scan runs, or a human `/cw-orchestrate` run overlapping another, stay **unsupported**: the guard cannot close that race, and this residual window is documented, not eliminated.
...
- **Concurrency model (no per-repo lock).** cw-orchestrate has no coarse run lock and does not use cw-ship's per-issue claim. In **number mode** it is launched **once per umbrella** behind an interactive readiness sweep, so the cross-run double-processing a claim guards against doesn't arise. In **repo-scan mode** (`/cw-orchestrate <owner>/<repo>`, the on-demand headless path) the label persists through an in-flight run, so before running a scan-picked umbrella the path applies a **live-state in-flight guard** (Step 1, Repo-scan mode): if the umbrella already has an in-flight orchestration (an open PR closing one of its sub-issues, or a sub-issue mid-work), it is **skipped this scan**. That guard — not a lock or mirror label — is how overlapping scans stay off each other. It is **best-effort**: it keys off signals a running orchestration emits, so the **launch→first-PR window** (before the first PR exists) is invisible to it. That window is safe only under a single-process sequential run; two concurrent repo-scan runs or a human run overlapping another remain unsupported (the guard cannot close that race — a documented residual, not a closed hole). Within a run, the **serial merge loop is the only mutual exclusion** (one node merges to the default branch at a time); the merge step re-checks `git merge-tree` against fresh `main` and rebases/retries when the base moved, so it never assumes a quiescent branch even while a cw-ship/cw-sweep run merges concurrently. Worktree cleanup is run-scoped by `workflowRunId` (Step 8), so concurrent runs never touch each other's artifacts. Running **two orchestrate runs on the same umbrella at once is unsupported** (they'd both process the same open sub-issues); the repo-scan guard makes the repo-scan path avoid it, and if per-sub-issue exclusion ever becomes a need, the per-issue claim from cw-ship's [state-machine.md](../cw-ship/references/state-machine.md) is the pattern to adopt.
