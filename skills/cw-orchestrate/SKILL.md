...
- [references/harness-portability.md](./references/harness-portability.md) — maps harness-specific capabilities to portable Agent Skills fallbacks
...
### Step 1: Resolve the target and enumerate open sub-issues

**Argument shapes (dispatch on the first argument):**

- **`<umbrella>` — a parent issue number or URL** (contains no `/`, or is a GitHub issue URL) → **number mode**. Runs the full interactive readiness sweep on one umbrella, exactly as documented below (Steps 2–8). This path's **interactive readiness sweep and human go-gate (Steps 2–4) are unchanged**. The one reconciliation delta that applies to **both** entry paths is Step 7's terminal removal of `cw-umbrella:ready` — a fully-resolved umbrella loses its pickup label in number mode too, not only in repo-scan mode (see [Step 7](#step-7-reconcile-the-umbrella-and-referenced-issues-github-is-the-source-of-truth)). Read the "unchanged" claim as scoping to the sweep and the go-gate, not to Step 7 reconciliation.
- **`<repo>` — an `owner/repo` slug** (contains a `/` and is not a full issue URL, e.g. `ALRubinger/craftwork-skills`) → **repo-scan mode**. Enumerates every OPEN issue in that repo carrying the `cw-umbrella:ready` label and runs each one, serially, through the per-umbrella flow — but with **no interactive sweep** (the label is the upstream clearance — human approval for a cw-scope umbrella, cw-ship's triage judgment for one it filed; see [Repo-scan mode](#repo-scan-mode-headless-label-driven-pickup) below). This is the entry point a non-interactive scheduler wrapper may drive (see cw-ship's [scheduling.md](../cw-ship/references/scheduling.md)).

Detect the shape from the argument itself: a bare number or an `.../issues/<n>` URL is **number mode**; an `owner/name` slug is **repo-scan mode**. When in doubt (e.g. an ambiguous token), treat a leading `#`/all-digits as number mode and a single `/`-joined `owner/name` as repo mode.

**Number mode** also accepts an optional scope filter:
...
An umbrella produced by cw-ship or cw-scope carries the `cw-umbrella:ready` state label (see [cw-ship's state machine](../cw-ship/references/state-machine.md)) — its scope was human-approved upstream and it is cleared for orchestration. Orchestrate consumes that label **read-only during pickup and the sweep** (it is the "ready for orchestration" marker and, in repo-scan mode, the enumeration key) and **removes it as a terminal reconciliation step once the umbrella is fully resolved** (Step 7). It never mirrors or re-stamps the label, and the native sub-issue graph (not the label) remains the single source of truth for what work exists — terminal removal reads the umbrella's live state, it does not introduce a second/mirror label.
...
### Repo-scan mode (headless, label-driven pickup)

the `cw-orchestrate` repo-scan mode for `<owner>/<repo>` runs **without** the interactive sweep. It trusts the upstream clearance the `cw-umbrella:ready` label already encodes (stamped at interactive scoping for cw-scope umbrellas, or by cw-ship's triage judgment that the feedback was umbrella-sized with clear intent — no `cw-feedback:go` gate, since a genuine design fork would have parked instead of producing an umbrella) and cannot solicit a human headless. This is the scheduled/opt-in entry path; the gate posture is documented in [readiness-sweep.md](./references/readiness-sweep.md#two-gate-postures).
...
1. **Enumerate ready umbrellas.** List every OPEN issue carrying the label:
   ...
   ```bash
   repo="$1"   # e.g. ALRubinger/craftwork-skills
   gh issue list --repo "$repo" --state open --label cw-umbrella:ready \
     --json number,title,state,labels --jq 'sort_by(.number)[].number'
   ```

   The pure enumerator is `pickReadyUmbrellas(issues)` in [scheduler.mjs](./scheduler.mjs) (OPEN + labeled, ascending). Empty list → nothing to do; report and exit.

2. **Per umbrella, apply the live-state in-flight guard.** Because the label persists through an in-flight run (it is only removed when the umbrella is fully resolved), a naïve scan could double-run an umbrella another orchestrate tick is already working. Before running a scan-picked umbrella, check its **live state** for an in-flight orchestration: an open PR that closes one of its sub-issues, or a sub-issue mid-work (e.g. an open linked PR). If one is found, **skip this umbrella this tick** and name it in the report as "in flight, skipped." Two orchestrate runs on one umbrella at once stays explicitly unsupported (as in number mode); this guard is how the scheduled path avoids it without introducing a lock or mirror label.

   **Best-effort, not a lock — the launch→first-PR window.** The guard keys off signals a *running* orchestration emits (an open PR, a sub-issue mid-work). Between launching a run and its first PR opening, no such signal exists yet, so the guard is **blind to a run in that window**. That launch→first-PR window is safe **only** under [scheduling.md](../cw-ship/references/scheduling.md)'s single-process sequential loop — one scheduled invocation at a time, each umbrella carried to completion before the next. Two concurrent schedulers, or a human `cw-orchestrate` run overlapping the scheduled loop, stay **unsupported**: the guard cannot close that race, and this residual window is documented, not eliminated.

3. **Run each surviving umbrella serially** through Steps 3b–8 below (the repo-mode variant of Steps 2–4, then Steps 5–8 unchanged). One umbrella at a time — the existing serial concurrency model.

#### Step 3b: Assemble the manifest headlessly (repo-scan mode; replaces Steps 2–4)

For a scan-picked umbrella, there is **no interactive readiness sweep, no `--only`/`--except` filter, and no "go" checkpoint** — the label *is* the go. Instead, per OPEN sub-issue:

1. **Classify readiness non-interactively.** Apply the same routing judgment the sweep uses ([readiness-sweep.md](./references/readiness-sweep.md)), but headless: read the sub-issue body and scan the repo for the decisions a plan would need.
   - A sub-issue that would route to **ready** (clear enough to plan as-is) gets a `route: ready` brief **derived non-interactively from its issue body** (per [manifest-schema.md](./references/manifest-schema.md)) and enters the manifest.
   - **A sub-issue whose body carries a recorded-answer block routes `ready`.** If the body has a `## Resolved fork` block (an `**Answer:**` line naming a settled fork, written by [the cw-resolve skill](../cw-resolve/SKILL.md) when it drained this sub-issue's `cw-status:stalled` park), the fork that stalled it is **resolved**: derive the `route: ready` brief from the body **plus that recorded answer** and enter it in the manifest. Do **not** re-classify the already-answered fork as a fresh unresolved fork and re-park it — that would loop the restored umbrella straight back to parked on every scan. (The sub-issue no longer carries `cw-status:stalled` by the time it is re-picked; the recorded answer is the durable resolution.)
   - A sub-issue that WOULD need **clarify-now** or **back-off-to-brainstorm** — an unresolved design fork a human would have to settle, with no recorded-answer block resolving it — is **parked immediately** and **excluded from the manifest**: `gh issue edit <n> --add-label cw-status:stalled` and upsert a `<!-- cw:status -->` comment `⏸ needs-input: <the unresolved fork>, parked for the cw-resolve skill` (consistent with the existing park model). It is never planned — the headless path must not plan against an unresolved fork. Draining that park is [the cw-resolve skill](../cw-resolve/SKILL.md)'s job (record the answer on the body, remove `cw-status:stalled`); the next scan then sees the recorded-answer block and routes it `ready`.
2. **Derive dependency edges from the umbrella's declared signals**, not an operator prompt: native "blocked by" relationships and any `depends_on`/"needs #NNN merged" signals in the sub-issue bodies. Record them as `depends_on` edges; prune any that name a parked/excluded sub-issue (with a report note, as in Step 3's out-of-scope pruning).
...
Then get an explicit **"go"** from the operator using the harness's blocking-question capability, or direct chat if no structured question UI exists. This is the last human checkpoint (R4). Show the routed list, the dependency edges, and the manifest path before asking, so the operator sees the run's blast radius before approving.
...
5. **Close the umbrella when it is fully done, and strip its pickup label.** After sub-issue state is reconciled, if **every** in-scope sub-issue is resolved (merged or closed) **and** none carries `cw-status:deferred`/`cw-status:stalled` **and** no residual is left `open — escalation` (closed/moot/autofixed residuals are fine), close the umbrella itself: `gh issue close <umbrella> --reason completed --comment "All sub-issues and residuals resolved. <one-line outcome>"`. Otherwise leave it **open** — an umbrella whose sub-issues are all closed should close, but any sub-issue still open (parked or in flight) or a live escalation means it is still tracking work. Never close an umbrella that still carries unresolved items.
   ...
   **Terminal transition of the umbrella pickup label.** The pickup label is a lifecycle marker in one of two mutually-exclusive states (`cw-umbrella:ready` ↔ `cw-umbrella:needs-input`), not a mirror of the sub-issue graph — orchestrate consumed `cw-umbrella:ready` read-only and now transitions it from the umbrella's **live state**. `readyLabelTerminalAction(umbrella)` in [scheduler.mjs](./scheduler.mjs) returns:
   ...
   - **`'remove'`** — strip `cw-umbrella:ready`, so a future scan never re-picks a done umbrella. Two cases:
     ...
     - **The umbrella is fully resolved but deliberately left open only for a still-live escalation residual** — every sub-issue is closed but a residual sits `open — escalation`. Sub-issue work is done, so orchestrate has nothing left to re-pick; the escalation is owned by the `cw-resolve` skill / cw-sweep, not by a re-scan.
...
- **The sweep is the only human touchpoint (number mode).** Everything after "go" is hands-off by design. If a sub-issue is not ready, resolve it in the sweep — do not rely on the headless planner to invent the missing decision. **Repo-scan mode has no sweep**: it is human-gated *upstream* at label-stamp time (the `cw-umbrella:ready` label is the approval), and a sub-issue that would need clarification is **parked** (`cw-status:stalled` + a needs-input comment) and excluded rather than guessed at — the headless path never plans against an unresolved fork. See [readiness-sweep.md](./references/readiness-sweep.md#two-gate-postures).
...
- **Concurrency model (no per-repo lock).** cw-orchestrate has no coarse run lock and does not use cw-ship's per-issue claim. In **number mode** it is launched **once per umbrella** behind an interactive readiness sweep, so the cross-run double-processing a claim guards against doesn't arise. In **repo-scan mode** (the `cw-orchestrate` repo-scan mode for `<owner>/<repo>`, the scheduled/opt-in path) the label persists through an in-flight run, so before running a scan-picked umbrella the path applies a **live-state in-flight guard** (Step 1, Repo-scan mode): if the umbrella already has an in-flight orchestration (an open PR closing one of its sub-issues, or a sub-issue mid-work), it is **skipped this tick**. That guard — not a lock or mirror label — is how overlapping scans stay off each other. It is **best-effort**: it keys off signals a running orchestration emits, so the **launch→first-PR window** (before the first PR exists) is invisible to it. That window is safe only under scheduling.md's single-process sequential loop; two concurrent schedulers or a human run overlapping the scheduled loop remain unsupported (the guard cannot close that race — a documented residual, not a closed hole). Within a run, the **serial merge loop is the only mutual exclusion** (one node merges to the default branch at a time); the merge step re-checks `git merge-tree` against fresh `main` and rebases/retries when the base moved, so it never assumes a quiescent branch even while a cw-ship/cw-sweep run merges concurrently. Worktree cleanup is run-scoped by `workflowRunId` (Step 8), so concurrent runs never touch each other's artifacts. Running **two orchestrate runs on the same umbrella at once is unsupported** (they'd both process the same open sub-issues); the repo-scan guard makes the scheduled path avoid it, and if per-sub-issue exclusion ever becomes a need, the per-issue claim from cw-ship's [state-machine.md](../cw-ship/references/state-machine.md) is the pattern to adopt.
...
- **GitHub stays the source of truth (Step 7).** A run reconciles every issue it touched before it ends: merged-but-open sub-issues are closed (squash-merge auto-closes only with a `Closes #NNN` keyword), parked sub-issues are labeled `cw-status:stalled`/`cw-status:deferred` with an upserted reason comment (the native sub-issue widget rolls up the rest), the umbrella itself is closed once every sub-issue and residual is resolved (left open otherwise), the `cw-umbrella:ready` pickup label is stripped once the umbrella is fully resolved (or closed) so a future scan never re-picks it, and the parent issue is updated (its umbrella line flips to done only when nothing is left deferred or stalled). Orchestrate consumes `cw-umbrella:ready` read-only during pickup/sweep and *removes* it as this terminal step — it never mirrors or re-stamps it, and removal reads the umbrella's live state (no second/mirror label). Anyone reading the issues sees the true state without the run logs.
