...
- **`<repo>` — an `owner/repo` slug** (contains a `/` and is not a full issue URL, e.g. `ALRubinger/craftwork-skills`) → **repo-scan mode**. Enumerates every OPEN issue in that repo carrying the `cw-umbrella:ready` label and runs each one, serially, through the per-umbrella flow — but with **no interactive sweep** (the label is the upstream human approval; see [Repo-scan mode](#repo-scan-mode-headless-label-driven-pickup) below). This is the entry point the optional `claude -p "/cw-orchestrate <owner>/<repo>"` scheduler step (cw-ship's [scheduling.md](../cw-ship/references/scheduling.md)) drives.
