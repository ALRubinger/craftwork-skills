...
- **Stalled sub-issue parks are not (yet) drained here — but clearing one auto-unblocks its umbrella.** When cw-orchestrate's headless repo-scan hits a sub-issue it can't plan against, it parks that **sub-issue** with `cw-status:stalled` (not a `## Decision needed` block), and if that leaves the umbrella with no runnable work it swaps the umbrella `cw-umbrella:ready` → `cw-umbrella:needs-input` so scans stop churning ([cw-orchestrate state machine](../cw-ship/references/state-machine.md)). This inbox drains `cw-feedback:needs-input` / `cw-review-residual:needs-input`, **not** `cw-status:stalled` sub-issues — release a stalled sub-issue by clearing its `cw-status:stalled` label (a hand-release today, like a hold). You do **not** touch `cw-umbrella:needs-input`: cw-orchestrate's next repo scan detects the cleared park and swaps the umbrella back to `cw-umbrella:ready` on its own (`needsInputTerminalAction` → `'restore'`).
