...
**`cw-ship` is an on-demand tool.** The expected way to run it is to ask your agent to use the `cw-ship` skill for `<owner>/<repo>` when you want the feedback backlog drained; it then plans, builds + merges the clear items, and pages you only when a decision is parked. It is **not** wired to run automatically, and scheduling it is a deliberate opt-in, not the default.
...
The recipes need a non-interactive agent command. The examples use `claude -p`; substitute your harness's equivalent command. Replace `<owner>/<repo>` and the checkout path with yours.
...
AGENT_CMD=(claude -p) # Replace with your harness's non-interactive command.
...
- **A fully-blocked umbrella leaves the scan set (no per-tick churn).** If an umbrella's only remaining open work is those parked sub-issues, a run has nothing to advance. Rather than keep `cw-umbrella:ready` and re-process it every tick, cw-orchestrate **swaps it to `cw-umbrella:needs-input`** as a terminal step, so `pickReadyUmbrellas` skips it until a human clears a park; once the blocking sub-issue loses `cw-status:stalled` (however released), the next scan's step-0 reconcile swaps it back to `cw-umbrella:ready`. Net: a blocked umbrella costs one scan to detect, then zero until it is unblocked.
...
**Permissions:** do not run headless with a permission mode that can stop for an interactive prompt. Pre-approve the `gh`/`git`/`gh api` operations the skill needs according to your harness's permission model. In Claude Code, for example, that means relying on `~/.claude/settings.json` allow-lists rather than passing a prompt-for-each-command mode. For other runtimes, ensure the agent can run `gh`/`git` non-interactively without prompting.
...
`cw-ship.cmd` at `%USERPROFILE%\bin\cw-ship.cmd` (ensure `git`, `node`, and your agent CLI are on the system PATH):
...
- **GitHub Actions cron.** A scheduled workflow runs the agent in CI — repo-native, runs regardless of any laptop, best for teams. Use your agent harness's CI action on a `schedule:` trigger. The runner needs a token with merge rights, and the bot identity must be allowed to bypass required review.
- **Managed cloud routine.** Runs on managed infra, no server or open session. Usually a minimum interval around an hour, and billed.

## Safety, however you schedule it

- Scope the agent's auth to the target repo, and start with a dry run — `cw-ship` accepts `build: false` to plan and park without opening PRs. Do one manual non-interactive run for your harness and watch it before activating any schedule.
