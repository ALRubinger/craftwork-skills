...
Clear the standing backlog of `cw-review-residual` issues that `cw-orchestrate` files.

Each `cw-review-residual` issue is a consolidated set of plan-review findings the umbrella run deferred. Filed against a **plan**, many are already moot by the time the feature ships, others are cheap correct fixes, and only a few are genuine decisions. This skill re-judges every finding against the **shipped code**, then:
...
When the user wants to triage, clear, or act on `cw-review-residual` issues — e.g. "triage the residual issues", "clear the cw-review-residual backlog", "act on the residuals for umbrella #989". Do **not** use it to triage residuals from a run currently in flight (cw-orchestrate handles those itself).
...
- **Whole backlog** — every open `cw-review-residual` issue in the repo.
...
1. **Discover** — list in-scope open `cw-review-residual` issues; map each to its tracked sub-issue (from the `Relates to #<n>` line), derive its **per-residual target branch** (see below), and read its labels to classify a `human_state`: `fresh`, `needs-input` (parked, awaiting the operator — skipped this run), or `go` (operator answered — triaged in consume mode).
2. **Triage** — one subagent per `fresh`/`go` residual, in parallel: re-judge each finding against the shipped diff + current files **on the residual's target branch**, post a triage comment, and close the residual if nothing actionable remains. A residual whose sub-issue hasn't shipped *on that target* is left open and marked deferred. For `go` residuals it runs in **consume mode**: it reads the operator's `**Answer:**` lines under the `## Decision needed` block and re-classifies accordingly (accept → resolved; "do X" → a high-confidence fix).
3. **Autofix** (if enabled) — over a now-quiescent target branch, each residual with a high-confidence `FIX_NOW` finding gets one PR (implementing only those fixes, forked from and opened against **its** target branch) serial-merged through the standard merge-safety contract. The PR `Closes` the residual when its fixes resolve everything actionable, else `Relates to` it.
...
#### Per-residual target branch (integration-branch aware)

A single sweep mixes residuals from **different** umbrellas, so the target branch is derived **per residual**, not once for the run. In Discover, each residual's umbrella is read for a `cw-target:<slug>` label and the target branch (`discover[].target_branch`) is derived inline by the same rules as [cw-orchestrate's `target.mjs` `deriveTarget`](../cw-orchestrate/target.mjs) — applied in prose against the gh-fetched labels, **not** by importing the module (`target.mjs` is a main-session helper, never loaded by a background Workflow):

- **One** `cw-target:<slug>` label on the umbrella → `integration/<slug>`.
- **Zero** labels (or no umbrella) → the default branch (behavior unchanged).
- **More than one** label, or an **empty/whitespace** slug → malformed umbrella: the residual degrades to the default branch and the operator is told to fix the label (a per-residual sweep does **not** abort every other residual over one bad umbrella, unlike `deriveTarget`'s hard error in a single-umbrella orchestrate run).

That `target_branch` is then the **ground truth** for the residual everywhere downstream: triage judges "shipped?" and every finding against `integration/<slug>` HEAD, and the autofix PR forks from and lands on `integration/<slug>`. **A residual whose integration branch does not exist yet defers** (shipped:false) exactly like an unshipped default-branch feature — a later sweep triages it once the integration branch carries the work. The whole feature is promoted from `integration/<slug>` to the default branch later by `/cw-promote`, not by this sweep.
...
```json
{
  ...
  "target_mismatches": [{ "residual_issue": 993, "sub_issue": 987, "target_branch": "integration/foo", "actual_base": "main" }]
}
```
...
3. Write the answer back into the residual body (the `## Decision needed` block) as an `**Answer:** <decision>` line, then advance the label: an accept/no-change answer → close the residual; a "do X" answer → `--add-label cw-review-residual:go --remove-label cw-review-residual:needs-input` so the next sweep applies it. Use `gh issue view … --json body -q .body > body.md` → edit → `gh issue edit … --body-file body.md` (never hand-escape). After clearing a batch, offer to re-run `/cw-sweep` on the `:go` set to apply the answered fixes now.
...
- **`deferred_residuals`** — residuals whose feature hasn't shipped on its target branch; nothing to do now, re-run after it merges (for an integration residual, after the work lands on `integration/<slug>`).
- **`target_mismatches`** — a **soft diagnostic**, not a decision: an autofix PR landed (or opened) against a base that disagrees with the residual's derived target branch (e.g. derived `integration/foo` but the PR based on `main`). Surface each as its own line — residual link, derived `target_branch`, the PR's `actual_base` — so the operator can check the fix went to the right branch. It does **not** block the run, is **not** in `escalations` (no operator decision is owed), and is **not** folded into an `autofixed[].cause`. An empty list is the normal case.
...
1. **Reconcile sub-issue state and the residual list.** Sub-issues are native sub-issues: one this run closed already shows closed in the umbrella's widget — no checkbox to flip; for any still-open sub-issue the run parked, set `cw-status:deferred`/`cw-status:stalled` and upsert its `<!-- cw:status -->` reason comment, and remove the label from any that resolved (issue-reconciliation rule 2). Then update the umbrella body's **residual follow-ups** list (residuals are separate issues, not sub-issues, so this prose is not a duplicate): for every residual this run closed (including via an autofix PR), mark it closed and update any line that still calls it "open" or "awaiting the residual-triage skill." Fetch the body with `gh issue view <umbrella> --json body -q .body > body.md`, edit surgically, and push with `gh issue edit <umbrella> --body-file body.md`. Preserve everything you did not change verbatim.
...
4. **Verify.** Re-read the umbrella: confirm every closed residual/sub-issue is reflected (residual list updated, the sub-issue widget shows the closed ones, no stale `cw-status:*` label on a resolved sub-issue), and that the only items still marked open are genuine deferrals or not-yet-shipped work. Quote the remaining open items back to the operator.
...
- **Integration-branch aware, per residual.** "Shipped" is relative to the residual's **target branch**, derived per residual from its umbrella's `cw-target:<slug>` label (one sweep mixes residuals from many umbrellas, so there is no single run target). A residual whose umbrella carries `cw-target:<slug>` is triaged, autofixed, and merged against `integration/<slug>` — never the default branch — and defers if that integration branch doesn't exist yet. Residuals with no `cw-target` label behave exactly as before (default branch). The derivation reuses cw-orchestrate's `deriveTarget` rules but applies them inline (no JS import) since the background Workflow can't load `target.mjs`. `target_mismatches[]` is the soft check that an autofix actually landed where it should have.
