...
//
// Every residual is judged against, and every autofix PR lands on, the repo's
// default branch ("shipped" means on the default branch; the autofix forks from
// and merges onto it).
//
  ...
  properties: {
    residuals: {
      ...
      items: {
        type: 'object',
        additionalProperties: false,
        required: ['residual_issue', 'residual_url', 'sub_issue', 'human_state'],
        properties: {
...
Repo ${a.repo}. GROUND-TRUTH BRANCH for this residual: ${a.defaultBranch} (the repo's default branch).
...
2. Determine whether the feature #${subIssue} has SHIPPED — i.e. its code is on \`${a.defaultBranch}\` now. Try these in order and stop at the first that resolves it:
   ...
   b. No closing-PR link does NOT mean unshipped — issues closed without a "Closes #" keyword have none. Search directly: \`gh pr list --repo ${a.repo} --state merged --search "${subIssue}"\` and \`git fetch origin ${a.defaultBranch} -q && git log --oneline --grep "#${subIssue}" origin/${a.defaultBranch}\`. A merged PR or merge commit referencing it on \`${a.defaultBranch}\` => shipped.
   ...
   Conclude NOT shipped ONLY when the feature is open/halted AND no merged PR or merge commit references it on \`${a.defaultBranch}\` AND its code is absent at \`${a.defaultBranch}\` HEAD (e.g. a P0-halted feature). Then set shipped:false, mark every finding "DECISION" with rationale "feature not yet shipped; cannot triage against code", do NOT close, and return. A future run triages it once the feature lands.
...
Steps:
1. Fetch the current body into a private temp dir (write all scratch here, never into the working checkout): \`D=\"\$(mktemp -d)\"; gh issue view ${tr.residual_issue} --repo ${a.repo} --json body -q .body > \"\$D/body.md\"\`.
2. Write the decisions into \"\$D/body.md\" as a "## Decision needed" block — one numbered entry per decision below, each showing the question, your recommended answer, and the alternatives. If a "## Decision needed" block already exists (a prior park), REPLACE it in place rather than appending a second one. The decisions:
\`\`\`json
${JSON.stringify(
...
Repo ${a.repo}. MERGE TARGET: \`${a.defaultBranch}\` (the repo's default branch). Implement EXACTLY these triaged fixes and nothing more:
\`\`\`json
...
if (runAutofix) {
  ...
  for (const residualIssue of queue) {
    ...
    log(
      `Autofix #${residualIssue}: ${merged ? 'merged' : 'stalled — ' + (mResult.cause || 'not green')}` +
