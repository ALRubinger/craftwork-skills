...
// cw-orchestrate/triage.mjs; the function bodies are kept identical so the
// two skills classify residuals the same way. `workflow.js` inlines a
// byte-for-byte mirror of these functions (a Workflow script cannot import at
// runtime); mirror.test.mjs guards the drift. Pure: no Date.now(),
...
/**
 * Findings that need a human: every DECISION, plus any low-confidence FIX_NOW.
 * @param {Array} results
 * @returns {Array}
 */
